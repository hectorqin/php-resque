...
class Parser
{
        ...
        if (in_array((int) date('i', $startTime), $date['minutes'])
            ...
            foreach ($date['second'] as $second) {
                if (\class_exists("\\Carbon\\Carbon")) {
                    $result[] = \Carbon\Carbon::createFromTimestamp($startTime + $second);
                } else {
    ...
    /**
     * @param null|\Carbon\Carbon|int $startTime
     */
    private function parseStartTime($startTime): int
    {
        if (\class_exists("\\Carbon\\Carbon") && $startTime instanceof \Carbon\Carbon) {
