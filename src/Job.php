...
class Job
{
    ...
    /**
     * @var object|\Resque\JobInterface Instance of the class performing work for this job.
     */
    private $instance;
     ...
     *
     * @param int $status Status constant from JobStatus indicating the current status of a job.
     */
     ...
     *
     * @return int The status of the job as one of the JobStatus constants.
     */
     ...
     * @return Resque\JobInterface Instance of the object that this job belongs to.
     ...
     * @return string
     */
    public function recreate($generateID = true)
    {
        $status  = new JobStatus($this->payload['id']);
        $monitor = false;
        ...
        return self::create($this->queue, $this->payload['class'], $this->getArguments(), $monitor, $generateID ? null : $this->payload['id']);
    }
    ...
    public function __toString()
    {
        ...
        if (!empty($this->payload['args'])) {
            $name[] = json_encode($this->payload['args'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        }
        return '(' . implode(' | ', $name) . ')';
    }
