...
use Resque\Excetion\InvalidTimestampException;
...
{
	const VERSION = "0.1";
	public static $delayQueueName = 'delayed_queue_schedule';
	...
	{
		return (int)Resque::redis()->zcard(self::$delayQueueName);
	}
	...
	private static function cleanupTimestamp($key, $timestamp)
	{
		...
		if ($redis->llen($key) == 0) {
			$redis->del($key);
			$redis->zrem(self::$delayQueueName, $timestamp);
		}
	}
	...
	public static function nextDelayedTimestamp($at = null)
	{
		...
		$items = Resque::redis()->zrangebyscore(self::$delayQueueName, '-inf', $at, array('limit' => array(0, 1)));
		if (!empty($items)) {
			return $items[0];
		}
